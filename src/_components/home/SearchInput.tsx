...
export default function SearchInput({ handleAddHistory }: { handleAddHistory: (s: SearchHistory) => void }) {
	...
	async function onClickSearchBtn(e: MouseEvent<HTMLButtonElement>) {
		...
		if (!/^[0-9-]+$/.test(userInput)) {
			setErrorMessage('郵便番号は半角数字のみまたは半角数字とハイフンのみで入力してください。');
			setIsFetching(false);
			setSearchResults([]);
			return;
		}
		...
		if (!/^(\d{7}|\d{3}-\d{4})$/.test(userInput)) {
			setErrorMessage('郵便番号は半角数字でハイフンありの8桁かハイフンなしの7桁で入力してください。');
			setIsFetching(false);
			setSearchResults([]);
			return;
		}
	...
	return (
		<>
			<section className="search-section">
				<input type="text" className="search-input" placeholder="111-1111" ref={postalInpRef} />
				<button type="button" className="search-button" onClick={onClickSearchBtn} disabled={isFetching}>
					検索
				</button>
			</section>
